        ...
        <div className="flex justify-center items-center gap-6 overflow-x-auto">
          {[
            ...
            <div key={idx}>
              {/* ✅ 웹/태블릿: 기존 원형 유지 */}
              <div
                className="hidden sm:flex items-center justify-center rounded-full border border-gray-300 text-center text-gray-700 p-4 
        w-40 h-40 sm:w-48 sm:h-48 text-sm sm:text-base flex-shrink-0"
              >
                {text}
              </div>

              {/* ✅ 모바일: 직사각형 + 글씨 크게 + 가운데 정렬 + 키워드 강조 */}
              <div
                className="sm:hidden flex items-center justify-center text-center 
        text-gray-700 text-base font-medium my-4 border border-gray-200 rounded-xl p-6"
              >
                {text
                  .replace("differentiation", "⚡ differentiation")
                  .replace("market presence", "⚡ market presence")
                  .replace("Unintuitive reviews", "⚡ Unintuitive reviews")}
              </div>
            </div>
          ))}
        </div>
