        ...
        <section className="mt-16">
          ...
          <section className="mt-16">
            ...
            <section className="mt-16">
              ...
              {/* 8-A. Hae-Mi’s Needs Point */}
              <section className="mt-10" id="haemi-needs">
                {/* 제목 */}
                <div className="mb-6">
                  <h3 className="text-xl font-bold text-purple-600">
                    Hae-Mi’s Needs Point
                  </h3>
                  <p className="text-sm text-gray-600 mt-1">
                    Personalized Meal Profile — We provide information tailored
                    to users.
                  </p>
                </div>

                {/* 본문: 웹=좌측 설명 + 우측 화면들 / 모바일=세로 스택 */}
                <div className="grid grid-cols-1 md:grid-cols-12 gap-6 items-start">
                  {/* Left panel (설명) */}
                  <aside className="md:col-span-4">
                    <div className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
                      <p className="text-3xl md:text-4xl font-extrabold leading-tight text-purple-700">
                        Personalized
                        <br />
                        Meal Profile
                      </p>
                      <p className="text-gray-600 mt-3">
                        We provide information tailored to users.
                      </p>

                      {/* 포인트 요약 – 필요 없으면 삭제해도 OK */}
                      <ul className="mt-5 space-y-2 text-sm text-gray-700">
                        <li>• 빠른 알러지 선택(다중 선택 지원)</li>
                        <li>• 프로필 완성 유도 카피 & 최소 탭</li>
                        <li>• 저장/완료 피드백 명확화</li>
                      </ul>
                    </div>
                  </aside>
                  ...
                  <div className="md:col-span-8">
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                      {/* Screen 1 */}
                      <figure className="rounded-2xl border border-gray-200 bg-white p-3 shadow-sm">
                        <img
                          src="/haemi-meal-profile-01.jpg" // ← 네 이미지로 교체
                          alt="Meal Profile – category list"
                          className="w-full h-auto rounded-xl object-contain"
                          loading="lazy"
                        />
                        <figcaption className="text-xs text-gray-500 mt-2">
                          Category-first allergy input
                        </figcaption>
                      </figure>
                      ...
                      <figure className="rounded-2xl border border-gray-200 bg-white p-3 shadow-sm">
                        <img
                          src="/haemi-meal-profile-02.jpg" // ← 네 이미지로 교체
                          alt="Fruit allergy multi-select"
                          className="w-full h-auto rounded-xl object-contain"
                          loading="lazy"
                        />
                        <figcaption className="text-xs text-gray-500 mt-2">
                          Fruit allergy: multi-select with short helpers
                        </figcaption>
                      </figure>
                      ...
                      <figure className="rounded-2xl border border-gray-200 bg-white p-3 shadow-sm">
                        <img
                          src="/haemi-meal-profile-03.jpg" // ← 네 이미지로 교체
                          alt="My Kurly Style – profile completion"
                          className="w-full h-auto rounded-xl object-contain"
                          loading="lazy"
                        />
                        <figcaption className="text-xs text-gray-500 mt-2">
                          Profile completion & privacy toggle
                        </figcaption>
                      </figure>

                      {/* Screen 4 (선택) */}
                      <figure className="rounded-2xl border border-gray-200 bg-white p-3 shadow-sm lg:col-span-1 sm:col-span-2">
                        <img
                          src="/haemi-meal-profile-04.jpg" // ← 네 이미지(저장 완료)로 교체
                          alt="Profile saved feedback"
                          className="w-full h-auto rounded-xl object-contain"
                          loading="lazy"
                        />
                        <figcaption className="text-xs text-gray-500 mt-2">
                          Clear “Profile saved” feedback
                        </figcaption>
                      </figure>
                    </div>
                  </div>
