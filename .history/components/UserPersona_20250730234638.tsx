...
export default function UserPersona() {
  return (
    <div className="bg-white rounded-2xl shadow-md p-8 max-w-6xl mx-auto space-y-8">
      {/* 상단 - 왼쪽 프로필 + 오른쪽 정보 */}
      <div className="grid grid-cols-3 gap-8">
        {/* Left: Profile */}
        <div className="col-span-1 text-center space-y-4">
          <Image
            src="/persona-koh.png" // 경민이 넣을 이미지 경로
            width={200}
            height={200}
            alt="Persona"
          ...
          <p className="text-purple-600 font-semibold">
            새로운 맛 조합 찾기 <br /> & 레시피 개발에 관심이 많아요
          </p>
          <p className="text-gray-500 text-sm">
            #호기심많음 #ENTJ #귀차니즘 <br /> #계획적 #화이트등급
          </p>
          <div className="bg-gray-100 p-3 rounded-xl text-gray-700 text-sm italic">
            "믿을 만한 리뷰가 필요하지만,
            <br /> 쓰는 건 귀찮아!"
          </div>
        </div>

        {/* Right: Behavior, Needs, Goal, Pattern */}
        <div className="col-span-2 space-y-6">
          {/* Behavior */}
          <div>
            <h3 className="text-purple-600 font-semibold mb-2">Behavior</h3>
      ...
      {/* 하단 Stage, Emotion, Pain Point */}
      <div className="bg-gray-50 p-6 rounded-xl space-y-4">
        <h3 className="text-purple-600 font-semibold">Stage & Emotion</h3>
        <p className="text-gray-600 text-sm mb-2">
          (여기는 차트 대신 구조만 잡아둔 상태, 추후 SVG로 감정 그래프 추가
          가능)
        </p>
        ...
        <div className="grid grid-cols-6 gap-4 text-xs text-gray-600 mt-4">
          <div>이 상품이 좋은지 확신이 들지 않는다</div>
          <div>맞는 리뷰 검색이 어렵다</div>
          <div>장바구니 담기 편한 방법 없음</div>
          <div>글 적는 게 귀찮다</div>
          <div>이 리뷰가 도움될까 고민</div>
          <div>리워드 기능 필요</div>
        </div>
      </div>
    </div>
